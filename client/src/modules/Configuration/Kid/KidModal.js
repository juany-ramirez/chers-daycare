...
import { Modal, Form, ToggleButtonGroup, ToggleButton, Row } from "react-bootstrap";
...
const KidModal = (props) => {
  ...
  const createUser = (values, { resetForm }) => {
    axios.post(`${process.env.REACT_APP_NODE_API}/api/auth/signup`, values)
      .then(response => {
        setValidation("");
  ...
  const modifyUser = (values) => {
    ...
    user.rol = values.rol;
    console.log(user);
    axios.put(`${process.env.REACT_APP_NODE_API}/api/auth/users/${user._id}`, user)
      .then(response => {
        console.log(response);
        if (response.data.success) {
  ...
  const submitForm = (values, { setSubmitting, resetForm }) => {
    setSubmitting(true);
    props.type === "create" ? createUser(values, { resetForm }): modifyUser(values, { resetForm });
    setSubmitting(false);
  }
  ...
  let content = (
    <div>
      ...
      {props.type === "create" && (
        <DButton
          title="Crear nuevo usuario" handleClick={() => setLgShow(true)} />
      )}
      <Modal
        size="lg"
        show={lgShow}
              ...
              /* and other goodies */
                ...
                <Form onSubmit={handleSubmit}>
                  ...
                  <Form.Group controlId="formUserType">
                    ...
                    >
                      <ToggleButton
                        variant="secondary" value={1}> Administrador </ToggleButton>
                      <ToggleButton
                        variant="secondary" value={2}> Caretaker </ToggleButton>
                      <ToggleButton
                        variant="secondary" value={3}> Padre/Madre </ToggleButton>
                    </ToggleButtonGroup>
                    <Form.Text className="text-muted">
